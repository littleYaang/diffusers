...
for dataset in "${!datasets[@]}"; do
    ...
    script_path="generated_scripts/eval_baseline-inpaintpipe_${dataset}.sh"
    log_file="${LOG_DIR}/000_${dataset}.log"

    if [ -f "$script_path" ]; then
        echo "Running eval_baseline-inpaintpipe_${dataset}.sh on GPU ${gpu_id}..."
...
for dataset in laion prim scut syn; do
    log_file="${LOG_DIR}/baseline-inpaintpipe_${dataset}.log"
    if [ -f "$log_file" ]; then
