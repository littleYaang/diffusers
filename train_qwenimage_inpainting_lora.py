...
class TextRemovalDataset(Dataset):
    """Dataset for text removal training with cached latents and embeddings"""
    ...
    def __init__(
        ...
        if metadata_path is None:
            metadata_path = self.latents_dir / "metadata.json"
    ...
    def __getitem__(self, idx):
        ...
        # Load cached latents
        latents_path = self.latents_dir / item["latents_file"]
        latents = load_file(latents_path)
        
        # Load cached text encoder outputs
        text_encoder_path = self.text_encoder_dir / item["text_encoder_file"]
        text_outputs = load_file(text_encoder_path)
        
        return {
            "clean_latents": latents["clean_latents"],
            "masked_latents": latents["masked_latents"],
            "prompt_embeds": text_outputs["prompt_embeds"],
            "prompt_embeds_mask": text_outputs["prompt_embeds_mask"],
        }


def collate_fn(examples):
    """Collate function for batching"""
    clean_latents = torch.stack([example["clean_latents"] for example in examples])
...
def main(args):
    ...
    # Create dataset with cached data
    train_dataset = TextRemovalDataset(
        latents_dir=args.latents_cache_dir,
        text_encoder_dir=args.text_encoder_cache_dir,
    )
